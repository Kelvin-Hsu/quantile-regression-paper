...
\label{sec:kernel_embeddings}
	...
	\label{sec:kernel_embeddings:theoretical_representation}
		...
		\begin{theorem} \label{thm:kernel_sum_rule}
			...
			\begin{equation}
				\muY = \Cylx \muX
			\label{equation:kernel_sum_rule}
			\end{equation}
			
			\begin{proof}
			\begin{align*}
				\Cylx \muX &= \Cylx \expect{\phiX} \tag{\cref{def:kernel_embedding}} \\
	  			&= \expect{\Cylx \phiX} \tag{linearity of $\expect{\cdot}$} \\
	  			&= \expect{\mu_{\rv{Y} | \rv{X}}} \tag{\cref{def:conditional_operator}} \\
	  			&= \expect{\expect{\psiY | \rv{X}}} \tag{\cref{def:conditional_embedding}} \\
	  			&= \expect{\psiY} \tag{tower property} \\
	  			&= \muY \tag{\cref{def:conditional_embedding}}
			\end{align*}
			\end{proof}
		\end{theorem}
...
%			\begin{equation}
%			\begin{aligned}
%				\hatmuX &= \frac{1}{n} \sum_{i = 1}^{n} \phi_{x_{i}} \otimes \phi_{x_{i}} \\
%				\hatmuY &= \frac{1}{n} \sum_{i = 1}^{n} \psi_{y_{i}} \otimes \psi_{y_{i}} \\
%			\label{eq:empirical_cross_cov}
%			\end{aligned}
%			\end{equation}
%		\end{theorem}
		
		The feature functions can be viewed as an feature vector of the same dimension as the cardinality of the its domain. For example, if the domain $\mathcal{X} = \mathbb{R}^{d}$ is the $d$ dimensional euclidean space whose cardinality is uncountably infinite, then the feature function can be viewed as an uncountably infinite dimensional feature vector, indexed by the elements of $\mathcal{X} = \mathbb{R}^{d}$. Denote the cardinality of $\mathcal{X}$ as $\cardX$, and similarly the cardinality of $\mathcal{Y}$ as $\cardY$. 
		
		With the observations $\{x_{i}, y_{i}\}_{i = 1}^{n}$ sampled from $\mathbb{P}_{\rv{X} \rv{Y}}$, there are $n$ feature vectors for each RKHS, $\{\phi_{x_{i}}\}_{i}^{n}$ for $\Hk$ and $\{\psi_{y_{i}}\}_{i}^{n}$ for $\Hl$. As elements within their respective RKHS for which Hilbert-Schmidt operators can operate on, $\phi_{x_{i}}$ has an effective dimension of $\cardX \times 1$ and $\psi_{y_{i}}$ has an effective dimension of $\cardY \times 1$.
		
		Recall that a matrix $A := \begin{bmatrix} \bvec{a}_{1} & \cdots & \bvec{a}_{n} \in \mathbb{R}^{n \times n}$, $\bvec{a}_{1} \in \mathbb{R}^{n} \; \forall i \in \{1, \cdots, n\}$, operated on a vector $\bvec{v} \in \mathbb{R}^{n}$ results in a vector that is the linear combination of the columns of $A$ with coefficients given by the components of $\bvec{v}$ \eqref{eq:matrix_operation}.
		...
		\begin{definition} \label{def:feature_matrix}
			...
			\begin{aligned}
				...
				\Psi \equiv \Phi_{\ds{Y}} &:= \begin{bmatrix} \psi_{y_{1}} & \cdots & \psi_{y_{n}} \end{bmatrix}
			\label{eq:feature_matrix}
			\end{aligned}
			\end{equation}
			
			Here, the subscript of $\Phi$ and $\Psi$ is dropped whenever it is clear from context what dataset the feature matrix is representing.
