...
\label{sec:kernel_embeddings}
	...
	\label{sec:kernel_embeddings:theoretical_representation}
		...
		\begin{theorem} \label{thm:kernel_sum_rule}
			(Kernel Sum Rule)
			The Kernel Sum Rule \eqref{equation:kernel_sum_rule} is the kernelised and non-parametric version of the basic sum rule of probability \eqref{eq:sum_rule}.
			\begin{equation}
			\begin{aligned}
				p_{\rv{Y}}(y) &= \int_{\mathcal{X}} p_{\rv{Y} | \rv{X}}(y | \cdot) d\mathbb{P}_{\rv{X}} \quad & \mathrm{(Lebesgue)} \\
				&= \int_{\mathcal{X}} p_{\rv{Y} | \rv{X}}(y | x) p_{\rv{X}}(x)dx \quad & \mathrm{(Riemann)}
			\label{eq:sum_rule}
			\end{aligned}
			\end{equation}
	...
	\label{sec:kernel_embeddings:empirical_representation}
		...
		Recall that a matrix $A := \begin{bmatrix} \bvec{a}_{1} & \cdots & \bvec{a}_{n} \end{bmatrix} \in \mathbb{R}^{n \times n}$, $\bvec{a}_{i} \in \mathbb{R}^{n} \; \forall i \in \{1, \cdots, n\}$, operated on a vector $\bvec{v} \in \mathbb{R}^{n}$ results in a vector that is the linear combination of the columns of $A$ with coefficients given by the components of $\bvec{v}$ \eqref{eq:matrix_operation}.
		...
		\begin{theorem} \label{def:kernel_matrix}
			(Kernel Gram Matrix)
			The gram matrix for a dataset can be found by its corresponding feature matrix \eqref{eq:kernel_matrix}.
			\begin{equation}
			\begin{aligned}
			...
			\begin{proof}
				Below is an informal proof for the case for the kernel $k$. The proof for the kernel $l$ is exactly the same.
				\begin{align*}
