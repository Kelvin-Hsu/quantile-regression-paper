...
%		\end{equation}
%		
%	Notice that this takes two arguments in $\mathcal{X}$. As such, when applied onto a function that takes one argument in $\mathcal{X}$, this acts like a matrix with size $\infty \times \infty$ (recall that all linear operators have respective matrix representations). Taking arguments becomes like indexing. For an usual finite dimensional matrix $A$, we usually write $A_{i j}$ for the element at the $i$-th row and $j$-th column. In programming, we usually write something like \texttt{A[i, j]} or \texttt{A(i, j)}. Now, this operator is indexed by the arguments instead, which are uncountably infinite, so we write $C_{\rv{X} \rv{X}}(x_{1}, x_{2})$ for example, which evaluates to be $\mathbb{E}[k(\rv{X}, x_{1}) k(\rv{X}, x_{2})]$.
%	
%	Now that we remember that all linear operators has their matrix representations, we can write 
%
%		\begin{equation}
%			C_{\rv{X} \rv{X}} := \mathbb{E}[\phi_{\rv{X}} \otimes \phi_{\rv{X}}] = \mathbb{E}[\phi_{\rv{X}} \phi_{\rv{X}}^{T}]
%		\end{equation}
%	
%	where we used the matrix notation for a tensor product between vectors. Now, when we apply this linear operator to a feature vector (which are functions in our case), we usually use the notation
%	
%		\begin{equation}
...
%		\end{equation}
%		
%	Noting that the first expression is an outer product applied to a vector, the second expression is a vector multiplied by a dot product which returns a scalar, and the third expression shows this notation readily makes associativity clear.
%
%	Of course, this is just notation, and this notation with 2-tensors at a time. With 3-tensors or higher tensor products, the original notation $\otimes$ would be more extendible.
%		
%	As an artificial example, we could have the following.
%	
%		\begin{equation}
...
%	
%	\begin{equation}
%		\Phi_{\ds{X}} := \begin{bmatrix} \phi_{x_{1}} & \dots & \phi_{x_{n}} \end{bmatrix} = (\{\phi_{x_{i}}\}_{i = 1}^{n})^{T}
%	\end{equation}
%	
...
%	\end{equation}
%	
%	We can quickly verify the dimensionality of this equation. On the left we have  $\hat{\mu}_{\rv{X}}$ being a $\infty \times 1$ vector. On the right we have $\Phi_{\ds{X}}$ being a $\infty \times n$ matrix applied on a $n \times 1$ vector $\bm{\beta}$, which also results in a $\infty \times 1$ vector.
%	
%\section{Standard Kernel Embedding Results and Derivations}
%
...
%		
%	
%		As always, let $\rv{X} : \Omega \mapsto \mathcal{X}$ be a random variable taking values in $\mathcal{X}$. Similarly, let $\rv{Y} : \Omega \mapsto \mathcal{Y}$ be a random variable taking values in $\mathcal{Y}$. Let $k$ and $l$ be characteristic positive definite kernels defined on $\mathcal{X}$ and $\mathcal{Y}$ respectively.\\
%		
%		Let $g \in \mathcal{H}_{l}$. Define $f \in \mathcal{H}_{k}$ as follows.
%		
%		\begin{equation}
%			f \equiv f(\cdot) := \mathbb{E}[g(Y) | X = \cdot]
%		\end{equation}
%		
%		Note that this is a function in $\mathcal{H}_{k}$ and the domain of $f$ is $\mathcal{X}$ as $f(x) = \mathbb{E}[g(Y) | X = x]$.
%	
%		\begin{equation}
%			\begin{aligned}
%				C_{\rv{X} \rv{X}} f &= \mathbb{E}[\phi_{\rv{X}} \phi_{\rv{X}}^{T}] f\\
%				&= \mathbb{E}[\phi_{\rv{X}} \phi_{\rv{X}}^{T} f] \\
%				&= \mathbb{E}[\phi_{\rv{X}} f(\rv{X})] \\
%				&= \mathbb{E}[\phi_{\rv{X}} \mathbb{E}[g(\rv{Y}) | \rv{X}]] \\
%				&= \mathbb{E}[\mathbb{E}[\phi_{\rv{X}} g(\rv{Y}) | \rv{X}]] \\
%				&= \mathbb{E}[\phi_{\rv{X}} g(\rv{Y})]
%			\end{aligned}
%		\end{equation}
	%	
