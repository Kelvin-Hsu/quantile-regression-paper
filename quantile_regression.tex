...
\label{sec:background}
	...
	\warn{This section is taken from the supplementary materials. Only the relevant bits are taken. The point is to have an expanded version of the background in the supplementary.}
...
%	
		\subsubsection{Theoretical Representations}
...
%			
			\begin{definition} \label{def:feature_matrix}
				(Feature Matrix)
				A feature matrix $\Phi \equiv \Phi_{\ds{X}}$ is formed by stacking the corresponding feature vectors \textit{horizontally}, where each feature vector represents a column of that matrix \eqref{eq:feature_matrix}. Here, the subscripts of $\Phi$ and $\Psi$ are dropped whenever it is clear from context the dataset involved.
