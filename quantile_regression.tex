...
%		Note that in most literature the cumulative distribution function would be denoted by $F_{X}$ and not $P_{X}$. However, with a lot of different spaces and variables to introduce later on, we will reserve $F$ and consequently $f$ for other quantities later.
	...
	\subsection{Kernel Embeddings}
		...
		\extra{It would indeed be more precise to put a subscript $k$ on the kernel embedding, as $\mu_{\rv{X}, k}$ or $\mu_{\rv{X}, k_{\theta}}$, to make clear that it was embedded with kernel $k$. However, as we will not be changing the kernel much except up to some hyperparameter tuning, this will just be implicitly assumed.}
