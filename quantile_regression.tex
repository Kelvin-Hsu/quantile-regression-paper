...
\begin{abstract}
	...
	Quantile regression is concerned with the estimation of quantiles from a conditional distribution of a response variable, offering a more complete and robust inference framework than the mean prediction. This is especially important in decision making applications where a risk probability is used to bound the acceptable ranges of the response variable. In this paper, we develop a nonparametric quantile regression framework by utilising the expressive power and flexibility of kernel embedding models, which embeds probability distributions as unique mean embeddings that can be represented directly from data. We present a set of novel algorithms for computing quantiles from such a nonparametric inference model, and compare it to state-of-the-art algorithms in quantile regression. On multiple standard datasets, our methods produce competitive or improved quantile estimates in comparison to other methods in the literature.
...
\section{Summary of Quantile Regression Algorithms}
