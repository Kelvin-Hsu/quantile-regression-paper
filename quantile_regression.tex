...
%		\end{theorem}
		...
		\begin{figure}
			...
			\end{center}
			\caption{\small (Left) The PDF obtained through the pre-image method from the Motorcycle dataset. The blue line indicates the PDF, and the red bars indicate the embedding weights at the training points. (Right) The resulting CDF estimate using \eqref{eq:empirical_cdf}. Note that the estimate jumps at locations corresponding to training points.}
